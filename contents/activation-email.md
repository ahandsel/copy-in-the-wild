---
title: Activation Email
description: Example of activation email for subscription services.
layout: 'doc'
outline: 'deep'
footer: 'true'
---

# Emails confirming activation of subscription


## Google Voice Starter subscription

**Subject line:**  
Thank you for activating your subscription

**Body:**

```md
## Your Google Voice Starter subscription has been activated

Hello,

Thank you for taking the steps to activate your Google Voice Starter subscription. You can continue using the available services. If you are looking for additional services, you can [compare Google Workspace editions](https://support.google.com/a/answer/6043385) or visit the Get more services page in the Google Admin console.

To prevent disruption of services, follow the steps below to keep your billing information up to date.

### Update billing information

1. Sign in to your [Google Admin console](https://admin.google.com/).
2. Go to **Billing > Payment accounts > View payment methods**.
3. Review your primary payment method and update if necessary. For more information on updating your payment method go to [Fix payment and billing issues](https://support.google.com/a/answer/2523116).

### Your Google Workspace account

> **Username:**  
> {user_email_address}
>
> **Domain:**  
> {domain_name}

[Sign in](https://admin.google.com/)

Sincerely,
The Google Workspace Team

© 2025 Google LLC 1600 Amphitheatre Parkway, Mountain View, CA 94043
_You're receiving this mandatory email service announcement to update you about important changes to your Google Workspace product or account._
```


## Claude Team invitation

**Subject line:**  
You're invited to use Claude with COMPANY_NAME

**Body:**

```md
# Use Claude with COMPANY_NAME

Invited by {claude_admin_email_address}

{claude_admin_email_address} is an admin inviting you to use Claude with them on the Team plan and get early access to new collaboration features.

[Accept Invitation]

By continuing, you agree to join the Team plan managed by your Admin, and acknowledge the Anthropic Privacy Policy. Learn more about the Team plan.
```
